...
$stmt = $pdo->prepare("SELECT * FROM cash_book WHERE date BETWEEN ? AND ? ORDER BY date, id");
$stmt->execute([$from, $to]);
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

$balance = 0;
$all = $pdo->query("SELECT * FROM cash_book ORDER BY date, id");
while ($r = $all->fetch(PDO::FETCH_ASSOC)) $balance += $r['type'] === 'in' ? $r['amount'] : -$r['amount'];

$openingBalance = 0;
$before = $pdo->prepare("SELECT * FROM cash_book WHERE date < ? ORDER BY date, id");
$before->execute([$from]);
while ($r = $before->fetch(PDO::FETCH_ASSOC)) $openingBalance += $r['type'] === 'in' ? $r['amount'] : -$r['amount'];

// Print view
if (isset($_GET['print'])) {
    ?><!DOCTYPE html><html lang="en"><head><title>Cash Book</title>
    ...
    <div class="print-box">
        <div class="header"><h1><?php echo e(getSetting($pdo, 'company_name') ?: 'Electric Bikes Showroom'); ?></h1><p>Cash Book</p></div>
        <div class="info">
            <span>Period: <?php echo $from; ?> — <?php echo $to; ?></span>
            <span class="fw-bold">Closing Balance: <?php echo formatMoney($balance); ?></span>
        </div>
        <table>
            <tr><th>Date</th><th>Description</th><th class="text-end">Cash In</th><th class="text-end">Cash Out</th><th class="text-end">Balance</th></tr>
...
<div class="content">
    <div class="topbar">
        <div><button class="sidebar-toggle" onclick="toggleSidebar()"><i class="bi bi-list"></i></button><span class="page-title">Cash Book</span></div>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover" data-skip-dt>
                        <thead><tr><th>Date</th><th>Description</th><th>Cash In</th><th>Cash Out</th><th>Balance</th></tr></thead>
                        <tbody>
