...
if ($cid) {
    ...
    if ($customer) {
        ...
        $ledger = $pdo->query("SELECT * FROM customer_ledger WHERE $where ORDER BY date, id");
        $balance = $customer['opening_balance'];
        $all = $pdo->query("SELECT * FROM customer_ledger WHERE customer_id=$cid ORDER BY date, id");
        while ($r = $all->fetch(PDO::FETCH_ASSOC)) $balance += $r['debit'] - $r['credit'];
    }
}

if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_entry'])) {
    $pdo->prepare("INSERT INTO customer_ledger (customer_id, date, description, debit, credit, balance) VALUES (?,?,?,?,?,0)")->execute([$_POST['customer_id'], $_POST['date'], $_POST['description'], $_POST['debit'] ?? 0, $_POST['credit'] ?? 0]);
    ...
    <div class="main-content">
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between">
                <span>Ledger: <strong><?php echo e($customer['name']); ?></strong></span>
            ...
            <div class="card-body p-0">
                <div class="table-responsive p-3">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr><td><?php echo $customer['created_at']; ?></td><td>Opening Balance</td><td>-</td><td>-</td><td class="fw-semibold"><?php echo formatMoney($customer['opening_balance']); ?></td></tr>
